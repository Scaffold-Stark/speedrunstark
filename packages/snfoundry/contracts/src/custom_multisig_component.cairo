...
// OpenZeppelin Contracts for Cairo v1.0.0 (governance/multisig/multisig.cairo)
...
pub mod MultisigComponent {
    use contracts::custom_interface_multisig_component::{
        IMultisig, TransactionID, TransactionState,
    };
    ...
    > of IMultisig<ComponentState<TContractState>> {
        ...
        fn add_signer(
            ...
            self._add_signer(new_quorum, signer_to_add);
        }
        ...
        fn submit_transaction(
            ...
            self.submit_transaction_batch(array![call], salt)
        }
        ...
        fn execute_transaction_batch(
            ...
            let id = self.hash_transaction_batch(calls.clone(), salt);
            match self.resolve_tx_state(id) {
        ...
        fn hash_transaction_batch(
            ...
            PedersenTrait::new(0).update_with(calls.span()).update_with(salt).finalize()
        }
    }
    ...
    > of InternalTrait<TContractState> {
        ...
        /// Emits a `QuorumUpdated` event if the quorum changes.
        fn initializer(
            ref self: ComponentState<TContractState>, quorum: u32, signer: ContractAddress,
        ) {
            self._add_signer(quorum, signer);
        }
        ...
        fn _add_signer(
            ...
            if !signer_to_add.is_zero() {
                ...
                if self.is_signer(signer_to_add) {
                    panic_with_felt252(Errors::ALREADY_A_SIGNER);
                }
