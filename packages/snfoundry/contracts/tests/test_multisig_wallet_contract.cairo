...
use test_event_utils::{
    build_confirm_revoked_event, build_quorum_updated_event, build_signer_events,
    build_signer_removed_event, build_tx_confirmed_event, build_tx_executed_event,
...
fn test_deploy_multisig() {
    ...
    // lets build events
    let signers_event = build_signer_events(
        signers.clone(), multisig_wallet_dispatcher.contract_address,
    );
    let quorum_update_event = build_quorum_updated_event(
        multisig_wallet_dispatcher.contract_address, 0, expected_quorum,
    );
...
fn test_single_transaction() {
    ...
    cheat_caller_address(
        multisig_wallet_dispatcher.contract_address, *signers.at(0), CheatSpan::TargetCalls(1),
    );
    let tx_id = multisig_wallet_dispatcher.submit_transaction(to, selector, calldata.span(), salt);
    let tx_submitted_event = build_tx_submitted_event(
        multisig_wallet_dispatcher.contract_address, tx_id, *signers.at(0),
    );
    ...
    // now we confirm the transaction -> Dont forget we set old quorum to 4 so we need to confirm
    // all of them.
    for i in 0..signers.len() {
        cheat_caller_address(
            multisig_wallet_dispatcher.contract_address, *signers.at(i), CheatSpan::TargetCalls(1),
        );
    ...
    let get_transaction_confirmations = multisig_wallet_dispatcher
        .get_transaction_confirmations(tx_id);
    ...
    let revoke_event = build_confirm_revoked_event(
        multisig_wallet_dispatcher.contract_address, tx_id, *signers.at(0),
    );
    ...
    multisig_wallet_dispatcher.confirm_transaction(tx_id);
    let tx_confirmed_event = build_tx_confirmed_event(
        multisig_wallet_dispatcher.contract_address, tx_id, *signers.at(0),
    );
    ...
    let tx_executed_event = build_tx_executed_event(
        multisig_wallet_dispatcher.contract_address, tx_id,
    );
    ...
    let replace_signer_tx_id = multisig_wallet_dispatcher
        .submit_transaction(to, replace_signer_selector, replace_signer_calldata.span(), 0);
    // confirm tx.
    for i in 0..signers.len() {
        cheat_caller_address(
            multisig_wallet_dispatcher.contract_address, *signers.at(i), CheatSpan::TargetCalls(1),
    ...
    multisig_wallet_dispatcher
        .execute_transaction(to, replace_signer_selector, replace_signer_calldata.span(), 0);
    let signer_removed_event = build_signer_removed_event(
        multisig_wallet_dispatcher.contract_address, old_signer,
    );
...
fn test_cannot_submit_tx_unauthorized() {
    ...
    // lets submit the transaction
    cheat_caller_address(
        multisig_wallet_dispatcher.contract_address, NEWSIGNER, CheatSpan::TargetCalls(1),
    );
...
fn test_cannot_submit_tx_twice() {
    ...
    // lets submit the transaction
    cheat_caller_address(
        multisig_wallet_dispatcher.contract_address, *signers.at(0), CheatSpan::TargetCalls(2),
    );
    let _ = multisig_wallet_dispatcher.submit_transaction(to, selector, calldata.span(), salt);
    let _ = multisig_wallet_dispatcher
        .submit_transaction(to, selector, calldata.span(), salt); //-> gonna fail 
}
...
mod test_event_utils {
    use openzeppelin_governance::multisig::MultisigComponent::{
        CallSalt, ConfirmationRevoked, Event as MultisigEvent, QuorumUpdated, SignerAdded,
        SignerRemoved, TransactionConfirmed, TransactionExecuted, TransactionSubmitted,
    };
    use starknet::ContractAddress;

    pub fn build_signer_events(
        signers: Array<ContractAddress>, contract: ContractAddress,
        ...
        for i in 0..signers.len() {
            arr
                .append(
                    (contract, MultisigEvent::SignerAdded(SignerAdded { signer: *signers.at(i) })),
                );
        }
    ...
    pub fn build_quorum_updated_event(
        contract: ContractAddress, old: u32, new: u32,
    ) -> Array<(ContractAddress, MultisigEvent)> {
        array![
            (
                contract,
                MultisigEvent::QuorumUpdated(QuorumUpdated { old_quorum: old, new_quorum: new }),
            ),
        ]
    }

    pub fn build_signer_removed_event(
        contract: ContractAddress, removed_signer: ContractAddress,
    ) -> Array<(ContractAddress, MultisigEvent)> {
        array![(contract, MultisigEvent::SignerRemoved(SignerRemoved { signer: removed_signer }))]
    }

    pub fn build_tx_submitted_event(
        contract: ContractAddress, tx_id: felt252, signer: ContractAddress,
    ) -> Array<(ContractAddress, MultisigEvent)> {
        array![
            (
                contract,
                MultisigEvent::TransactionSubmitted(
                    TransactionSubmitted { id: tx_id, signer: signer },
                ),
            ),
        ]
    }
        ...
        array![
            (
                ...
                MultisigEvent::TransactionConfirmed(
                    TransactionConfirmed { id: tx_id, signer: signer },
                ),
            ),
        ]
    }
        ...
        array![
            (
                ...
                MultisigEvent::ConfirmationRevoked(
                    ConfirmationRevoked { id: tx_id, signer: signer },
                ),
            ),
        ]
    }
    ...
    pub fn build_tx_executed_event(
        contract: ContractAddress, tx_id: felt252,
    ) -> Array<(ContractAddress, MultisigEvent)> {
        array![(contract, MultisigEvent::TransactionExecuted(TransactionExecuted { id: tx_id }))]
    }
    // event CallSalt for non-zero salt.
    pub fn build_call_salt_event(
        contract: ContractAddress, tx_id: felt252, salt: felt252,
    ) -> Array<(ContractAddress, MultisigEvent)> {
        array![(contract, MultisigEvent::CallSalt(CallSalt { id: tx_id, salt: salt }))]
    }
}
...
//  ✅ fn remove_signers(ref self: TState, new_quorum: u32, signers_to_remove:
