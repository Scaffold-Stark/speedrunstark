...
use snforge_std::{
    CheatSpan, EventSpyAssertionsTrait, Token, TokenImpl, TokenTrait, cheat_caller_address,
    set_balance, spy_events,
};
...
fn create_batch_call(
    to: ContractAddress, func_name: FunctionName, address_array: Array<ContractAddress>,
) -> Array<Call> {
    ...
    for i in 0..signers.len() {
        ...
        calls.append(Call { to: to, selector: selector, calldata: calldata.span() });
    }
    calls
}
...
fn test_add_signer_batch() {
    ...
    let calls = create_batch_call(
        multisig_dispatcher.contract_address, FunctionName::AddSigner, signers.clone(),
    );
...
fn test_transfer_funds() {
    ...
    let multisig_wallet_dispatcher = IMultisigWalletDispatcher {
        contract_address: multisig_dispatcher.contract_address,
    };

    multisig_wallet_dispatcher.transfer_funds(RANDOM_ENTITY, 1_000_000_u256);
    // Read the balance
...
fn test_is_confirmed() {
    ...
    let signers = create_signer_array();
    let calls = create_batch_call(
        multisig_dispatcher.contract_address, FunctionName::AddSigner, signers,
    );
...
fn test_quorum_change() {
    ...
    let tx_submitted_event = build_tx_submitted_event(
        multisig_dispatcher.contract_address, tx_id, OWNER,
    );
    let confirm_tx_event = build_tx_confirmed_event(
        multisig_dispatcher.contract_address, tx_id, OWNER,
    );
    let quorum_update_event = build_quorum_updated_event(
        multisig_dispatcher.contract_address, 1, 4,
    );
...
fn test_remove_signer() {
    ...
    let confirmation_revoked_event = build_confirm_revoked_event(
        multisig_dispatcher.contract_address, tx_id, OWNER,
    );
    let remove_signer_event = build_signer_removed_event(
        multisig_dispatcher.contract_address, removed_signer,
    );
...
fn test_cannot_submit_tx_unauthorized() {
    ...
    cheat_caller_address(
        multisig_wallet_dispatcher.contract_address, SIGNER1, CheatSpan::TargetCalls(1),
    );
...
fn test_cannot_submit_tx_twice() {
    ...
    // lets submit the transaction
    cheat_caller_address(
        multisig_wallet_dispatcher.contract_address, OWNER, CheatSpan::TargetCalls(2),
    );
    let _ = multisig_wallet_dispatcher.submit_transaction(to, selector, calldata.clone(), salt);
    let _ = multisig_wallet_dispatcher
        .submit_transaction(to, selector, calldata, salt); //-> gonna fail 
}
