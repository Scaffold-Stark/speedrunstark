  ...
  const priceData = sortedEvents.reduce<DataPoint[]>((acc, event, idx) => {
    const price =
      event?.eventName === "PriceUpdated"
        ? 1 /
          (Number(formatEther(event?.parsedArgs?.price || 0n)) / strkPriceInUSD)
        : 0;
    const borrowRate =
    ...
    return [
      ...acc,
      {
        ...
        borrowRate:
          borrowRate >= 0 && Number.isFinite(borrowRate)
            ? borrowRate
            : prevBorrowRate,
        savingsRate:
          savingsRate >= 0 && Number.isFinite(savingsRate)
            ? savingsRate
            : prevSavingsRate,
      },
    ];
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis
                ...
                scale="linear"
                domain={[
                  (dataMin: number) => dataMin - 0.0001,
                  (dataMax: number) => dataMax + 0.0001,
                ]}
                stroke={strokeColor}
